...
// ── Activity heatmap — GitHub-style horizontal layout ────────────────────────
// Columns = weeks (oldest left), rows = days of week (Mon top → Sun bottom)
function Heatmap({ history, rank }) {
  const today = new Date();

  // Build 7 rows × N columns grid. Each column is one week (Mon=0 … Sun=6).
  // We show up to 9 weeks (63 days) so it fits a phone width nicely.
  const WEEKS = 9;
  const DAYS  = WEEKS * 7; // 63 cells max

  // Align so the rightmost column ends on today's day-of-week
  const todayDow = (today.getDay() + 6) % 7; // Mon=0, Sun=6
  // Total cells to show: fill complete weeks to the left
  // cells[0] = oldest, cells[DAYS-1] = today
  const cells = [];
  ...
  // Build columns: group cells by week column index
  // First cell might not start on Monday — pad with nulls
  const cols = [];
  let col = [];
  // Pad the first column so day-of-week alignment is correct
  const firstDow = cells[0].dow;
  for (let p = 0; p < firstDow; p++) col.push(null); // empty padding at top
    ...
    <div>
      <div style={{ display:'flex', gap:GAP, overflowX:'auto', paddingBottom:4 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:GAP, marginRight:2, flexShrink:0 }}>
          {DAY_LABELS.map((l, i) => (
            <div key={i} style={{
              height:CELL, width:10, fontSize:8,
              color:'var(--text-muted)', lineHeight:`${CELL}px`,
            ...
            {col.map((cell, ri) => (
              <div key={ri}
                ...
                style={{
                  ...
                  background: cell ? cellBg(cell.h) : 'transparent',
                  cursor: cell?.h ? 'pointer' : 'default',
                  transition: 'transform 0.1s',
                  flexShrink: 0,
                }}
                onMouseEnter={e => { if (cell?.h) e.target.style.transform='scale(1.3)'; }}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginTop:8, flexWrap:'wrap' }}>
        {[
          ...
          { label:'Poor',    bg:'rgba(251,146,60,0.55)' },
          { label:'Penalty', bg:'rgba(248,113,113,0.65)' },
          { label:'None',    bg:'rgba(255,255,255,0.05)' },
        ].map(l => (
  ...
  if (!editing) return (
    <button
      ...
      style={{
        ...
        marginTop:4, padding:0, flexShrink:0,
